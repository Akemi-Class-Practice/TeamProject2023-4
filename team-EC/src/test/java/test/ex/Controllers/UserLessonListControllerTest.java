...
public class UserLessonListControllerTest {
	...
	public void testGetLessonListPage_Success() throws Exception{
		...
		// /user/lesson/listへのPOSTリクエストを作成する
		RequestBuilder request = MockMvcRequestBuilders.get("/user/lesson/list");
		
		// mockMvcを使用してリクエストを実行する。
		mockMvc.perform(request)
		.andExpect(status().isOk())							// レスポンスのHTTPステータスが200 (OK) であることを検証する。
		.andExpect(view().name("user-lesson-list.html"))	// レスポンスのビュー名が "user-lesson-list.html"であることを検証する。 
